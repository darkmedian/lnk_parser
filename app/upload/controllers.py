...
def parse_lnk(filename):
    ...
    if vol_flags[:2] == "10":
        ...
        output["volume_target"] = "Target is on local volume"
        ...
        output["volume_label"] = str(vol_label)
        #------------------------------------------------------------------------
    ...
    # Network Volume Table
        ...
        if(net_share_name_loc != 20):
            output["volume_network_error"] = "[!] Error: NSN ofset should always be 14h"
            sys.exit(1)

        net_share_name_loc = net_vol_off + net_share_name_loc
        net_share_name = read_null_term(f,net_share_name_loc)
        output["volume_share_name"] = str(net_share_name)
        ...
        if(net_share_mdrive != 0):
            ...
            output["mapped_drive"] = str(net_share_mdrive)
    else:
        output["volume_error"] = "Error: unknown volume flags"
        sys.exit(1)
    ...
    output["remaning_path"] = str(rem_data)
    #------------------------------------------------------------------------
    ...
    if flags[3]=="1":
         addnl_text,next_loc = add_info(f,next_loc)
         output["relative_pth"] = str(addnl_text.decode('utf-16be', errors='ignore'))
         next_loc = next_loc + 1

    if flags[4]=="1":
         addnl_text,next_loc = add_info(f,next_loc)
         output["working_dir"] = str(addnl_text)
         next_loc = next_loc + 1

    if flags[5]=="1":
         addnl_text,next_loc = add_info(f,next_loc)
         output["command_line"] = str(addnl_text.decode('utf-16be', errors='ignore'))
         next_loc = next_loc + 1

    if flags[6]=="1":
         addnl_text,next_loc = add_info(f,next_loc)
         output["icon_filename"] = str(addnl_text.decode('utf-16be', errors='ignore'))
    return output
